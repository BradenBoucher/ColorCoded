...
enum NoteheadDetector {
    ...
    private static func detectStaffRects(inCG cg: CGImage) -> [CGRect] {
        ...
        let minHeight = max(36, h / 22)

        var staffBands = findBands(rowInk, minVal: bandMin, minHeight: minHeight)
        if staffBands.isEmpty {
            staffBands = [(0, max(1, h - 1))]
        } else {
            staffBands = mergeBands(staffBands, gap: 14)
        }
